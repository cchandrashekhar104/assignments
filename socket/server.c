...
if(ch=='Y')
{
for(i=0;i<k;i++)			//looping through all the child processes
kill(pid[i],SIGUSR1);		//sending kill command to all the child process
exit(0);
}
else
printf("Continuing..\n");
...
void chld_handler()
{
close(new_fd);		//closing all the open connections
exit(0);
}
    ...
    while(1) {  // main accept() loop
        ...
        if (!pid[k]) { // this is the child process
			...
			while(1)
			{
				...
				if(numbytes==0)		//checking if connection is closed
				{
